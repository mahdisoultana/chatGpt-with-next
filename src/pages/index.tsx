...
function Home() {
  const order = { right: ' rounded-tr-none', left: ' rounded-tl-none order-1' };
  const isMe = (i: number) => i % 2 == 0;
  return (
    <div className="min-h-screen w-full bg-gray-900 flex items-center justify-center text-white">
      <article className="relative max-w-xl w-full min-h-screen rounded  m-auto py-6 px-4 space-y-4 pb-24">
        {[1, 2, 3, 42, 35, 23, 4].map((i) => (
          <div className="flex  justify-end  items-start " key={i}>
            {true ? (
              <p
                className={` ${
                  isMe(i) ? order.left : order.right
                } rounded-xl pb-4 text-sm tracking-wide    p-1 pl-3 w-full bg-gray-800`}
              >
                files when using them in your projects.
              </p>
            ) : (
              <audio
                controls
                src={'https://mp3.chillhop.com/serve.php/?mp3=17087'}
                className={`${
                  isMe(i) ? order.left : order.right
                } block bg-white rounded-xl pb-6 text-md  p-2 w-full  `}
              ></audio>
            )}
            <span
              className={` block w-10  h-10 ${
                isMe(i) ? 'rounded-md' : 'rounded-full'
              }  flex-shrink-0 mx-2`}
            >
              {isMe(i) ? (
                gptIcon
        ...
        >
          <input className="w-full outline-none  h-full bg-gray-100  p-4 text-lg" />
          <ButtonRecorder />
        </form>
      </article>
    </div>
